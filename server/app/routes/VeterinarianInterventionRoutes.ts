...
router.get('/', checkIfAuthenticated, getAuthUser, checkIfPermitted("pet_hist_veto", Method.GET), async (req, res) => {
    const interventions = await interventionController.getAllInterventions()
    res.json(interventions)
})

router.get('/:id', checkIfAuthenticated, getAuthUser, checkIfPermitted("pet_hist_veto", Method.GET), async (req, res) => {
...
router.post('/', checkIfAuthenticated, getAuthUser, checkIfPermitted("pet_hist_veto", Method.POST), async (req, res) => {
    const newIntervention = await interventionController.createIntervention(req.body)
    res.status(201).json(newIntervention)
})

router.put('/:id', checkIfAuthenticated, getAuthUser, checkIfPermitted("pet_hist_veto", Method.PUT), async (req, res) => {
...
router.delete('/:id', checkIfAuthenticated, getAuthUser, checkIfPermitted("pet_hist_veto", Method.DELETE), async (req, res) => {
