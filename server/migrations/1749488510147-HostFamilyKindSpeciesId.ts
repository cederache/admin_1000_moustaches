...
export class HostFamilyKindSpeciesId1749488510147
  implements MigrationInterface
{
  name = "HostFamilyKindSpeciesId1749488510147";

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`host_family_kind\` ADD \`speciesId\` int NULL`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`host_family\` DROP COLUMN \`canIsolate\``
    );
    await queryRunner.query(
      `ALTER TABLE \`host_family\` ADD \`canIsolate\` tinyint NULL DEFAULT NULL`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`host_family_kind\` ADD CONSTRAINT \`FK_860c13a5d3f16519f32bb309d4b\` FOREIGN KEY (\`speciesId\`) REFERENCES \`species\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
    );
    // Cats
    await queryRunner.query(
      `UPDATE \`host_family_kind\`SET speciesId=1 WHERE id=1 OR id = 2 OR id = 4 OR id = 7`
    );
    // Dogs
    await queryRunner.query(
      `UPDATE \`host_family_kind\`SET speciesId=2 WHERE id=3 OR id = 5`
    );
    // Rabbits
    await queryRunner.query(
      `UPDATE \`host_family_kind\`SET speciesId=3 WHERE id=6`
    );
    // Others (NAC)
    await queryRunner.query(
      `UPDATE \`host_family_kind\`SET speciesId=4 WHERE id=8 OR id=9`
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`host_family_kind\` DROP FOREIGN KEY \`FK_860c13a5d3f16519f32bb309d4b\``
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`host_family\` ADD \`canIsolate\` varchar(255) NULL`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`host_family_kind\` DROP COLUMN \`speciesId\``
    );
  }
}
